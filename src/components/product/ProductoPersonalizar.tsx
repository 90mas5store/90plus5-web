...
export default function ProductoPersonalizar({ product, breadcrumb, initialRelated = [] }: ProductoPersonalizarProps) {
    ...
    useEffect(() => {
        ...
        fetch(`/api/analytics/share?slug=${encodeURIComponent(producto.slug)}`)
            .then(r => r.json())
            .then(data => { if (data.count > 0) setShareCount(data.count); })
            .catch(() => {});
    }, [producto.slug]);
                        ...
                        <div className="pt-4 space-y-2">
                            ...
                            <div className="flex gap-2">
                                <button
                                    type="button"
                                    onClick={handleShare}
                                    className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl border border-white/10 hover:border-white/25 text-gray-500 hover:text-white transition-all text-xs font-bold uppercase tracking-widest"
                                >
                                    {copied ? (
                                        <>
                                ...
                                <button
                                    ...
                                    className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl border border-[#25D366]/20 hover:border-[#25D366]/50 text-[#25D366]/60 hover:text-[#25D366] transition-all text-xs font-bold uppercase tracking-widest"
                                >
