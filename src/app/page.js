    ...
    async function fetchData() {
      try {
        const [featuredRes, configRes] = await Promise.all([
          fetch("https://script.google.com/macros/s/AKfycbx3RYRMJ8vz_cfc_jhZh3t6FciJ3iszAMh83enMkv8dvBmz8uQonQ_nMCkBVK7jQDNkwg/exec?action=getFeatured"),
    ...
    <main className="bg-black text-white min-h-screen relative overflow-hidden">
      ...
      <header className="fixed top-0 left-0 w-full flex items-center justify-between p-6 z-30 backdrop-blur-md bg-black/30 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <span className="text-white text-lg font-semibold tracking-widest">90+5 Store</span>
        </div>
      ...
      {/* 🎯 CARRUSEL DE LIGAS */}
      <section id="ligas" className="px-4 pb-12 max-w-6xl mx-auto">
        <h2 className="text-center text-3xl font-semibold mb-8 text-[#E50914]">
          Ligas disponibles
        </h2>
        ...
        <div className="text-center mt-6">
          <a
            href={`/catalogo${ligaSeleccionada ? `?liga=${encodeURIComponent(ligaSeleccionada)}` : ""}`}
            ...
            {destacadosFiltrados.map((item) => (
              ...
              >
                ...
                <div className="p-4 text-center">
                  ...
                  <button className="w-full mt-3 py-2 rounded-full bg-primary hover:bg-accent text-white font-semibold transition-all">
                    Ver más
                  </button>
                </div>
              </motion.div>
